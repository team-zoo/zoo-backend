...
module.exports = Router() 
  ...
  .get('/:id', ensureAuth([roles.OWNER, roles.VISITOR]), (req, res, next) => {
    Visitor
      ...
      .populate('favoriteAnimal', { name: true })
      .then(foundVisitor => {
        if(!foundVisitor) return next(new HttpError(404, 'No visitor Found with this Id'));
  ...
  .patch('/:id', ensureAuth([roles.OWNER]), (req, res, next) => {
    ...
    Visitor
      ...
      .populate('favoriteAnimal', { name: true })
      .then(updatedVisitor => {
        if(!updatedVisitor) return next(new HttpError(404, 'No visitor Found with this Id'));
        res.send(updatedVisitor);
      })
      .catch(next);
  })

  .get('/stats/average-age-all', ensureAuth([roles.OWNER]), (req, res, next) => {
    Visitor
      .getAvgAgeAllZoos()
      .then(avgs => avgs ? res.send(avgs) : next(new HttpError(404, 'No averages found')))
      .catch(next);
  })

  .get('/stats/average-age-each', ensureAuth([roles.OWNER]), (req, res, next) => {
    Visitor
      .getAvgAgeEachZoo()
      .then(avg => avg ? res.send(avg) : next(new HttpError(404, 'No average found')))
      .catch(next);
  })

  .get('/stats/zoos-by-visitor-count', ensureAuth([roles.OWNER]), (req, res, next) => {
    Visitor
