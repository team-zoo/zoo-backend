...
module.exports = Router()
  ...
  .get('/stats/fav-animal-given-zoo/:id', ensureAuth([roles.OWNER]), (req, res, next) => {
    const { id } = req.params;
    Zoo
      .find({ id: `${id}` })
      .then(zoo => {
        console.log('zoo', zoo);
        console.log('zoo id', zoo[0]._id);


        Visitor
          .getGivenZoo(zoo[0]._id)
          .getMostFavoritedAnimalGivenZoo(zoo[0]._id)
          .then(givenZoo => {
            console.log(givenZoo);
            res.send(givenZoo);
          });
      })
      .catch(next);
